
import as4 from '../../images/as4.png'
import i1 from '../../images/i1.png'
import i2 from '../../images/i2.png'
import i3 from '../../images/i3.png'
import i4 from '../../images/i4.png'
import i5 from '../../images/i5.png'
import i6 from '../../images/i6.png'
import as from '../../images/as.png'
import as2 from '../../images/as2.png'
import as3 from '../../images/as3.png'
  export var data = [{
    img: i1,
    heading: '24/7 Support',
    para: 'Mauris placerat viverra nisl, eu pretium mauris bibendum a. Mauris pellente sque nec quam ac molestie.'



  },
  {
    img: i2,
    heading: 'Advanced Platform',
    para: 'Mauris placerat viverra nisl, eu pretium mauris bibendum a. Mauris pellente sque nec quam ac molestie.'

  },

  {
    img: i3,
    heading: 'High Performance',
    para: 'Mauris placerat viverra nisl, eu pretium mauris bibendum a. Mauris pellente sque nec quam ac molestie.'

  },

  {
    img: i4,
    heading: 'Competitive Commisions',
    para: 'Mauris placerat viverra nisl, eu pretium mauris bibendum a. Mauris pellente sque nec quam ac molestie.'

  },

  {
    img: i5,
    heading: 'Good Investment',
    para: 'Mauris placerat viverra nisl, eu pretium mauris bibendum a. Mauris pellente sque nec quam ac molestie.'

  },

  {
    img: i6,
    heading: 'Reliable Order Execution',
    para: 'Mauris placerat viverra nisl, eu pretium mauris bibendum a. Mauris pellente sque nec quam ac molestie.'

  }



  ]
  export var data2 = [{
    img: as,
    heading: '24/7 Support',
    para: 'Mauris placerat viverra nisl, eu pretium mauris bibendum a. Mauris pellente sque nec quam ac molestie.'



  },
  {
    img: as2,
    heading: 'Advanced Platform',
    para: 'Mauris placerat viverra nisl, eu pretium mauris bibendum a. Mauris pellente sque nec quam ac molestie.'

  },

  {
    img: as3,
    heading: 'High Performance',
    para: 'Mauris placerat viverra nisl, eu pretium mauris bibendum a. Mauris pellente sque nec quam ac molestie.'

  },

  {
    img: as4,
    heading: 'Competitive Commisions',
    para: 'Mauris placerat viverra nisl, eu pretium mauris bibendum a. Mauris pellente sque nec quam ac molestie.'

  },



  ]